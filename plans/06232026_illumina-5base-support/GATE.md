...
- **FASTA input — DONE** (`-f`/`--fasta`, SE + PE): the 5-Base drivers read 2-line records (`read_one_record`) and synthesize a Phred-40 (`I`) quality so the inverted call / `--five_base_baseq` masking path is uniform (masking is then a no-op — no real per-base quality). minimap2 reads FastA natively; the prior `requires FASTQ input in v1` guard is lifted. Hermetic-tested (`five_base_se_fasta_input_inverts_polarity`).
- `--multicore` for the duplex/consensus post-passes — **still pending**: the consensus/duplex passes are a streaming two-pass over the BAM whose dominant cost is BGZF decompression (a separately-tracked future extension), `rayon` is an optional dep absent from the default build, and the passes are experimental/preview (not byte-gated), so parallelizing them is deferred rather than risk output nondeterminism on a non-bottleneck path.
- **#1035 nit 1b — DONE**: `--five_base_min_mapq` now also drops low-MAPQ reads from the PRIMARY per-read BAM (SE + PE), not only the consensus/duplex pass, matching DRAGEN's MAPQ<20 alt-read filter. Hermetic-tested (`five_base_min_mapq_filters_primary_bam`).
- **#1035 nit 2 — DONE**: `--five_base_umi_len` now runtime-checks the inline-UMI soft-clip precondition (read-5' soft-clip ≥ `umi_len`, orientation-aware), warning loudly once + tallying violations so a misconfigured aligner fails loudly instead of silently mis-calling 5' methylation. Unit-tested (`five_base_softclip_helpers`, `five_base_umi_softclip_violation_detects_missing_clip`).
